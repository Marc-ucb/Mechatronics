...
# ==== Periodic servo test (toggle every PERIOD_SEC) (NEW) ====
SERVO_TEST_ENABLE     = True     # set False to disable the periodic test
SERVO_TEST_DEG        = 30       # how far to “raise” (CCW)
SERVO_TEST_PERIOD_SEC = 10       # raise… 10s …lower… 10s …

def _servo_test_task():
    # small delay so Arduino prints "READY" etc. before we start
    time.sleep(2.0)
    while SERVO_TEST_ENABLE:
        send_servo_raise(SERVO_TEST_DEG)   # CCW by SERVO_TEST_DEG
        time.sleep(SERVO_TEST_PERIOD_SEC)
        send_servo_lower()                 # CW back to home
        time.sleep(SERVO_TEST_PERIOD_SEC)
...
def main():
    ...
    # --- start non-blocking 10s servo toggle test (NEW) ---
    if SERVO_TEST_ENABLE:
        threading.Thread(target=_servo_test_task, daemon=True).start()
