...
def get_pixy_steering():
    ...
    try:
        ...
        # Midpoint x of each vector
        line_xs = [(vec['x0'] + vec['x1']) / 2 for vec in vectors]
        line_xs.sort()

        robot_center = 79 / 2  # Pixy2 frame width / 2  (adjust if needed)
        gain = 2.5

        # Two or more lines: use left/right as edges and aim for center
        if len(line_xs) >= 2:
            left_edge = line_xs[0]
...
# Simple validity + target distance for side-wall fusion
MIN_VALID_MM = 50
MAX_VALID_MM = 1500
TARGET_SIDE_DIST_MM = 300   # tune: desired distance to the "far" wall
K_TOF_STEER = 0.05          # tune: how strongly ToF affects steering
...
def interpret_data(r, l, fr, fl):
    ...
    if pixy_result is not None:
        ...
        # =========================
        # Mode 2: One line + opposite ToF
        # =========================
        elif mode == "one":
            tof_extra = 0.0

            # If line is on the right, use LEFT ToF as "far" wall
            if edge_side == "right":
                if tof_valid(l):
                    # positive (l - target) -> steer left
            ...
            # If line is on the left, use RIGHT ToF as "far" wall
            elif edge_side == "left":
                if tof_valid(r):
                    # positive (r - target) -> steer right (negative)
    ...
    # Fallback to original TOF-based steering when Pixy has no lines
