...
# Try a set of likely locations (your diagnostics showed some of these)
_candidate_paths = [
    "/src/host/libpixyusb2_examples/python_demos",
    "/home/pi/pixy2/build/python_demos",
    "/home/smashbot/pixy2/build/python_demos",
    "/home/smashbot/pixy2/build/python_demos",  # repeated from diag
    "/usr/local/lib/python3.11/dist-packages",
    # also try relative to this script (useful when repo is checked out near the code)
...
pixy = None
pixy_mod = None  # module that actually exposes init/ccc_get_blocks/etc
BlockArray = None
VectorArray = None
...
try:
    ...
    try:
        ...
        # by default use this as the module for high-level functions
        pixy_mod = pixy
    except Exception as e:
        pixy = None
        pixy_mod = None
        # keep going to try compiled module
    ...
    try:
        ...
        # maybe the compiled .so is named _pixy at top-level
        try:
            _swig_mod = importlib.import_module("_pixy")
        except Exception:
            _swig_mod = None

    # Resolve classes / functions from available modules (top-level pixy or swig module)
    ...
    for m in sources:
        ...
        # prefer pixy (top-level) for high-level calls if it has them
    ...
    if BlockArray is None:
        try:
            # try loading module from file name used in your diagnostics
...
if not _pixy_available:
    print("[WARN] Pixy2 Python module not available or missing expected symbols (BlockArray/VectorArray/line functions). Pixy functionality will be skipped.")
else:
    print("[OK] Pixy module appears available (pixy_mod=%s, BlockArray=%s, VectorArray=%s)" % (
...
def init_pixy2():
    ...
    global pixy_blocks, pixy_vectors, pixy_mod
    if not _pixy_available:
    ...
    try:
        if hasattr(mod, "init"):
            ...
            # some wrappers place init in the swig module
            if hasattr(pixy, "init"):
                pixy.init()

        # switch program to line (if available)
        try:
...
def detect_color_blocks():
    ...
    - 3 = Purple
    """
    global last_detected_color, color_detection_timestamp, pixy_blocks, pixy_mod
    ...
    try:
        if hasattr(pixy_mod, "ccc_get_blocks"):
            count = pixy_mod.ccc_get_blocks(50, pixy_blocks)
            ...
            if _sw and hasattr(_sw, "ccc_get_blocks"):
                count = _sw.ccc_get_blocks(50, pixy_blocks)
            else:
                return None
    except Exception as e:
...
def get_pixy_steering():
    ...
    try:
        # Update line features then grab vectors (use available function)
