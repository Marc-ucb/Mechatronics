...
# Voltage thresholds for analog IR sensors (for future use)
...
def read_ir_digital():
    ...
    Returns: (left, right) boolean tuple
    """
...
# =============================================================================================================
# IR-based Track Correction (MATCHES DIAGNOSTIC LOGIC) ========================================================
# =============================================================================================================

def handle_ir_correction():
    """
    Simple IR correction, using the SAME logic as your diagnostic script:
      ...
      Cases:
        L=True,  R=True  -> ON_TRACK      -> no correction (return False)
        L=True,  R=False -> too far RIGHT -> GO LEFT  (steer left)
        L=False, R=True  -> too far LEFT  -> GO RIGHT (steer right)
    ...
    # L=True, R=False  -> left on track, right on bumper -> robot too far RIGHT -> steer LEFT
    if left_digital and not right_digital:
        send_set_vel_pwm(0, 0)
        send_set_vel_pwm(-100, 100)   # pivot left
        time.sleep(0.1)
        state_history(71)
        return True

    # L=False, R=True -> right on track, left on bumper -> robot too far LEFT -> steer RIGHT
    if right_digital and not left_digital:
        send_set_vel_pwm(0, 0)
        send_set_vel_pwm(100, -100)   # pivot right
        time.sleep(0.1)
        state_history(72)
        return True

    # L=False, R=False -> BOTH_LIGHT -> OFF TRACK
    robotState(7)
    state_history(73)
    return True
...
def interpret_data(r, l, fr, fl):
    ...
    with IR correction given PRIORITY.
    """
