...
def _send_line(line: str):
    """Send a single ASCII command line to Arduino, or print if serial not available."""
...
def send_set_vel(left_norm: float, right_norm: float):
    """Clamp to [0,1] and send SET_VEL."""
    L = max(0.0, min(1.0, left_norm))
    R = max(0.0, min(1.0, right_norm))
    _send_line(f"SET_VEL L={L:.2f} R={R:.2f}")

def send_turn_90(direction: str):
    """direction: 'LEFT' or 'RIGHT'"""
    if direction not in ("LEFT", "RIGHT"):
        return
    _send_line(f"TURN {direction} 90")

# ===== Open-loop speed & steering tuning (edit to taste) =====
BASE_SPEED   = 0.70   # normalized 0..1 for nominal straight motion
SLOW_SCALE   = 0.50   # front slow-down multiplier when approaching
K_STEER      = 0.70   # how strongly we reduce inside motor at max slope (0..1)
SLOPE_NORM   = 600.0  # |sDiff| (mm/s) that maps to "full" steering reduction

TURN_COOLDOWN_MS = 900  # don't resend turn command within this window
...
def _steer_reduction_from_slope(slope_mmps: float) -> float:
    """
    Map |sDiff| to how much we reduce the inside motor in dynamic turns.
    0 -> no reduction; 1 -> max reduction (K_STEER).
    """
    mag = abs(slope_mmps)
    x = max(0.0, min(1.0, mag / SLOPE_NORM))
    return K_STEER * x  # fraction to subtract from inside motor

def _apply_speed_scale(v: float, scale: float) -> float:
...
def read_dual_sensors():
    ...
    decision = 0  # -2..+2
    absDiff = 0.0
    sDiff = 0.0   # we also keep slope for dynamic steering magnitude
    ...
    if present(m1) and present(m2):
        ...
        # keep decision as 0 if we can't compute this cycle
        pass
    ...
    if present(mF):
        ...
        # front OOR -> keep normal speed
        pass

    # -------- COMMAND ARDUINO --------
    # map decision to commands
    now_ms = millis()
    if not hasattr(read_dual_sensors, "lastTurnAtMs"):
        read_dual_sensors.lastTurnAtMs = 0

    # Hard turns -> send one TURN command, then wait cooldown (Arduino should execute and resume)
    if decision == -2 or decision == +2:
        if (now_ms - read_dual_sensors.lastTurnAtMs) >= TURN_COOLDOWN_MS:
            dir_str = "LEFT" if decision == -2 else "RIGHT"
            send_turn_90(dir_str)
            read_dual_sensors.lastTurnAtMs = now_ms
        # During cooldown we don't spam SET_VEL; Arduino handles turn then continues.
        return

    # Straight / Dynamic steering -> send SET_VEL
    base = BASE_SPEED
    base = _apply_speed_scale(base, speed_scale)

    if decision == 0:
        # STRAIGHT
        send_set_vel(base, base)
    elif decision == -1:
        # DYNAMIC LEFT: right same, left decreases with |sDiff|
        red = _steer_reduction_from_slope(sDiff)  # fraction
        left  = base * (1.0 - red)
        right = base
        send_set_vel(left, right)
    elif decision == +1:
        # DYNAMIC RIGHT: left same, right decreases with |sDiff|
        red   = _steer_reduction_from_slope(sDiff)
        left  = base
        right = base * (1.0 - red)
        send_set_vel(left, right)
    else:
        # UNKNOWN state -> safest is stop or straight at reduced speed.
        # We'll choose a gentle straight at current base (you can change to stop if desired).
        send_set_vel(base * 0.5, base * 0.5)
