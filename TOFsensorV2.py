...
def read_dual_sensors():
    ...
    if present(m1) and present(m2):
        ...
        if not havePrevPair:
            ...
            # Engage when |diff| >= DIFF_ON_MM; release only when |diff| <= DIFF_OFF_MM.
            ...
            # 3) Combine:
            # Prefer slope (when active), otherwise position (with latch).
            decision = d_slope if d_slope != 0 else d_pos
