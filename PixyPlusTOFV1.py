...
# ==========================================================================================================
# Robot State Machine + Turn Modes =========================================================================
# ==========================================================================================================
...
# ---- High-level modes ----
MODE_NORMAL = 0
MODE_TURN_RIGHT_90 = 2
MODE_TURN_LEFT_90 = 3

current_mode = MODE_NORMAL
mode_start_time = 0.0

# Turn durations (seconds) – tune these on the real robot
TURN_RIGHT_DURATION = 0.5
TURN_LEFT_DURATION = 0.4


def start_turn_right_90():
    """Start a non-blocking ~90° right turn."""
    global current_mode, mode_start_time
    current_mode = MODE_TURN_RIGHT_90
    mode_start_time = time.time()
    send_set_vel_pwm(-255, 255)  # spin right in place
    state_history(2)
    print("Begin Right 90 turn")


def start_turn_left_90():
    """Start a non-blocking ~90° left turn."""
    global current_mode, mode_start_time
    current_mode = MODE_TURN_LEFT_90
    mode_start_time = time.time()
    send_set_vel_pwm(255, -255)  # spin left in place
    state_history(3)
    print("Begin Left 90 turn")


def update_turn_mode():
    """
    Called every loop from driving().
    If we're in a turn mode, check elapsed time and finish the turn when done.
    """
    global current_mode, mode_start_time

    if current_mode == MODE_TURN_RIGHT_90:
        elapsed = time.time() - mode_start_time
        if elapsed >= TURN_RIGHT_DURATION:
            # stop turn and roll forward
            send_set_vel_pwm(0, 0)
            send_set_vel_pwm(64, 64)
            current_mode = MODE_NORMAL
            print("End Right 90 turn")

        # While turning we still read sensors each loop,
        # but we don't run normal steering logic.
        return True

    if current_mode == MODE_TURN_LEFT_90:
        elapsed = time.time() - mode_start_time
        if elapsed >= TURN_LEFT_DURATION:
            # stop turn and roll forward
            send_set_vel_pwm(0, 0)
            send_set_vel_pwm(64, 64)
            current_mode = MODE_NORMAL
            print("End Left 90 turn")

        return True

    # Not in a turning mode
    return False


def robotState(state: int):
    """
    Discrete routines for obstacle / bridge / ramp / gravel.
    90° turns (2 / 3) are now handled via non-blocking turn modes instead.
    """
    full = 255
    half = 255 / 2
    ...
    match state:
        case 1:
            # Obstacle routine (still blocking for now)
...
def interpret_data(r, l, fr, fl):
    ...
    if pixy_correction is not None:
        ...
        if r < 100:
            # force robot to not slow right motor too much (bias away from right wall)
            right_speed = max(right_speed, base_speed - 20)

        if l < 100:
            # force robot to not slow left motor too much (bias away from left wall)
...
def driving():
    ...
    while True:
        ...
        # ==== 90° TURN MODE UPDATE ====
        # If we're currently in a turning mode, update it and skip normal steering.
        if update_turn_mode():
            # Still in a timed turn; sensors are read but we don't alter motor cmd here.
            continue

        # Combined decision logic (ToF + Pixy2) when not in a turn
        interpret_data(fR, fL, fFR, fFL)

        # Optional small pause to avoid maxing CPU (tweak as needed)
        # time.sleep(0.005)
